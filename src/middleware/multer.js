...
function getMulterMiddleware(fieldName, destinationPath, allowVideo, numberOfFiles = 1) {
        ...
        try {
            const storage = multer.diskStorage({
                ...
                filename: (req, file, cb) => {
                    ...
                    if (fileType[0] === 'image') {
                        const fileName =
                            'img_' + new Date().toISOString().replace(/:/g, '-') + '_' + nanoid() + '.' + fileType[1];
                        cb(null, fileName);
                        file.fileRelativeUrl = destinationPath + fileName;
                    } else if (fileType[0] === 'video') {
                        const fileName =
                            'video_' + new Date().toISOString().replace(/:/g, '-') + '_' + nanoid() + '.' + fileType[1];
                        cb(null, fileName);
                        file.fileRelativeUrl = destinationPath + fileName;
                    }
                },
            ...
            let multerHandler = multer({
                limits: { files: numberOfFiles },
                storage,
                fileFilter: (req, file, cb) => {
                    const imageTest =
                        imageRegex.test(extname(file.originalname).toLowerCase()) && imageRegex.test(file.mimetype);
                    const videoTest =
                        videoRegex.test(extname(file.originalname).toLowerCase()) && videoRegex.test(file.mimetype);

                    if (allowVideo === false && videoTest) {
                        ...
                        error['allowedMediaTypes'] = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif'];
            ...
            multerHandler(req, res, (err) => {
                if (err) {
                    ...
                    if (err instanceof multer.MulterError && err.code === 'LIMIT_FILE_COUNT') {
                        req.multerError = `cannot upload more than ${numberOfFiles} files`;
                        res.status(400).json({
                            errors: [
                                {
                                    msg: req.multerError,
...
function handleMulterError(req, res, next) {
    ...
    if (req.file) {
        const imageTest =
            imageRegex.test(extname(req.file.originalname).toLowerCase()) && imageRegex.test(req.file.mimetype);
        const videoTest =
            videoRegex.test(extname(req.file.originalname).toLowerCase()) && videoRegex.test(req.file.mimetype);
